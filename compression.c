...
void to_string(char * string, transaction_t transaction){
    int i = 0;
    i += sprintf(&string[i], "%d", transaction.user_id); 
    strcat(string, " ");
...
void decompress_to_file(){
    ...
    /* If fp is NULL, print error message and finish */ 
    if(rfp == NULL){
        printf("Transactions not loaded. Is database in right folder? \n");
    } 
    char ch;
    while(1){
...
transaction_t * read_database(int *tr_size){
    FILE * fp;
    long lsize;
    char * buffer;
...
/*    printf("%s", buffer);*/
    ...
    char * transaction; 
    transaction = strtok(buffer, "\n");
    while(transaction != NULL){
        sscanf(transaction, "%d %s %d", &transactions[i].user_id, transactions[i].item_name,
